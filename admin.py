    ...
    def login(self, Event=None):
        ...
        if results:
            if results[0][6] == "Admin":
                name = results[0][1]  # get the user's name from the database
                messagebox.showinfo("Login Page", f"Welcome, {name}!")
                # clear the username and password fields
                page1.entry1.delete(0, END)
                page1.entry2.delete(0, END)

                # hide the login window and open the admin page
                root.withdraw()
                global adm
                global page2
                adm = Toplevel()
...
def about():
        ...
        flask_process = subprocess.Popen(
            ['python', 'hello.py'])  # Replace 'your_flask_app.py' with your actual Flask app filename
...
class Admin_Page:
    def __init__(self, top=None):
        ...
        self.button5.place(relx=0.732, rely=0.504, width=146, height=63)
        ...
        self.button5.configure(font="-family {Poppins SemiBold} -size 13")
        self.button5.configure(borderwidth="0")
        self.button5.configure(text="""Analyze\nstore""")
                                ...
                                if pmrp:
                                    try:
                                        ...
                                        if valid_phone(pvendor):
                                            with sqlite3.connect("./Database/store.db") as db:
                                                cur = db.cursor()
                                                cur.execute("SELECT 1 FROM raw_inventory WHERE product_name=?",
                                                            (pname,))
                                                if cur.fetchone():
                                                    ...
                                                    insert = (
                                                        "INSERT OR IGNORE INTO raw_inventory(product_name, product_cat, product_subcat, stock, mrp, cost_price, vendor_phn) VALUES(?,?,?,?,?,?,?)")
                                                    cur.execute(insert, [pname, pcat, psubcat, int(pqty), float(pmrp),
                                                                         float(pcp), pvendor])
                                                    db.commit()
                                    ...
                                    try:
                                        ...
                                        if valid_phone(pvendor):
                                            with sqlite3.connect("./Database/store.db") as db:
                                                cur = db.cursor()
                                                cur.execute(
                                                    "SELECT 1 FROM raw_inventory WHERE product_name=? AND product_id!=?",
                                                    (pname, valll[0]))
                                                if cur.fetchone():
                                                    messagebox.showerror("Oops!",
                                                                         "Product already exists in the inventory.",
                                                    ...
                                                    cur.execute(update, [pname, pcat, psubcat, int(pqty), float(pmrp),
                                                                         float(pcp), pvendor, valll[0]])
                                                    db.commit()
                                                    messagebox.showinfo("Success!!",
        ...
        if ename.strip():
            if valid_phone(econtact):
                if valid_aadhar(eaddhar):
                    if edes:
                        if eadd:
                            if epass:
                                with sqlite3.connect("./Database/store.db") as db:
                                    cur = db.cursor()
                                    cur.execute("SELECT 1 FROM employee WHERE aadhar_num=?", (eaddhar,))
                                    if cur.fetchone():
                                        messagebox.showerror("Oops!", "Employee already exists in the database.",
                                                             parent=e_add)
                                    else:
                                        ...
                                        db.commit()
                                        messagebox.showinfo("Success!!",
                                                            "Employee ID: {} successfully added in database.".format(
                                                                emp_id), parent=e_add)
                                        self.clearr()
                            else:
                    ...
                    messagebox.showerror("Oops!", "Invalid Aadhaar number.", parent=e_add)
            else:
                    ...
                    if edes:
                        if eadd:
                            if epass:
                                with sqlite3.connect("./Database/store.db") as db:
                                    cur = db.cursor()
                                    cur.execute("SELECT 1 FROM employee WHERE aadhar_num=? AND emp_id!=?",
                                                (eaddhar, vall[0]))
                                    if cur.fetchone():
                                        messagebox.showerror("Oops!",
                                                             "Employee with this Aadhaar card number already exists in the database.",
                                                             parent=e_update)
                                    else:
                                        update = (
                                            "UPDATE employee SET name = ?, contact_num = ?, address = ?, aadhar_num = ?, password = ?, designation = ? WHERE emp_id = ?"
                                        )
                                        cur.execute(update, [ename, econtact, eadd, eaddhar, epass, edes, vall[0]])
                                        db.commit()
                                        messagebox.showinfo("Success!!",
                                                            "Employee ID: {} successfully updated in database.".format(
                                                                vall[0]), parent=e_update)
                                        vall.clear()
