...
def login(Event=None):
    ...
    if results:
        ...
        # hide the login window and open the billing window
        root.withdraw()
        global biller
        global page2
        biller = Toplevel()
        ...
        if self.state == 1:
            ...
            if(cust_name.get()==""):
                ...
                if strr.find('Total')==-1:
                    ...
                    # Construct the invoice content
                    invoice_content = f" {store_name}\n {store_address}\nDate: {current_date}\n\n{self.Scrolledtext1.get('1.0', END)}"
                    ...
                    # Add image
                    image_path = 'images/docxbill.png'
                    document.add_picture(image_path, width=Inches(6), height=Inches(2))

                    # Add invoice content
                    document.add_paragraph(invoice_content)

                    # Save DOCX file
                    docx_file_path = f'invoice_{cust_new_bill.get()}.docx'
                    document.save(docx_file_path)

                    # Print the invoice using os.startfile
                    try:
                        os.startfile(docx_file_path, 'print')
